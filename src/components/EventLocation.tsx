...
export const EventLocation: React.FC<EventLocationProps> = ({ address }) => {
    ...
    <section className="flex flex-col items-start gap-4 self-stretch relative">
      ...
      <div className="flex items-start gap-8 self-stretch relative max-sm:flex-col max-sm:gap-4">
        ...
        <a
          ...
          aria-label={`Open directions to ${address} on Google Maps (opens in a new tab)`}
          className="text-[#1A1A1A] text-[11px] font-normal uppercase relative bg-transparent cursor-pointer hover:opacity-70 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1A1A1A] focus-visible:ring-offset-2 focus-visible:ring-offset-white transition-opacity flex items-center gap-2 no-underline whitespace-nowrap"
        >
          <img src={arrowRight} alt="" aria-hidden="true" className="w-2 h-2.5" />
          GET DIRECTIONS
        </a>
      </div>
      <iframe
